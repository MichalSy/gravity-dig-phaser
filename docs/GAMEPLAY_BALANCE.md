...
Der Forschungsgraph besteht aus vier klar getrennten, geschwungenen Lanes um The Bucket. Die jeweilige Hauptfolge bleibt im Überblick durchgehend sichtbar und kreuzungsfrei. Querverbindungen zwischen Forschungsfamilien werden erst hervorgehoben, wenn einer ihrer Endpunkte ausgewählt ist. Mehrere Knoten besitzen zwei eingehende Routen und entweder alternative (`any`) oder gemeinsam erforderliche (`all`) Voraussetzungen. Dadurch können Wege zusammenlaufen, ohne die Gesamtansicht mit dauerhaft sichtbaren Brückenlinien zu überladen. Alte Upgrade-IDs, gespeicherte Käufe, Kosten und Gameplayeffekte bleiben kompatibel. Käufe werden weiterhin zentral im `PlayerStateManager` validiert.
