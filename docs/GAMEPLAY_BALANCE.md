...
Der Forschungsgraph besteht aus vier klar getrennten, orthogonalen Wegen. Alle benachbarten Hauptknoten besitzen exakt denselben Abstand, und jede Hauptverbindung verläuft waagerecht oder senkrecht. Querverbindungen zwischen Forschungsfamilien werden erst hervorgehoben, wenn einer ihrer Endpunkte ausgewählt ist. Mehrere Knoten besitzen zwei eingehende Routen und entweder alternative (`any`) oder gemeinsam erforderliche (`all`) Voraussetzungen. Dadurch können Wege zusammenlaufen, ohne die Gesamtansicht mit dauerhaft sichtbaren Brückenlinien zu überladen. Alte Upgrade-IDs, gespeicherte Käufe, Kosten und Gameplayeffekte bleiben kompatibel. Käufe werden weiterhin zentral im `PlayerStateManager` validiert.
