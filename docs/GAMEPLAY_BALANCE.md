...
## Dunkle Forschungs-Sternenkarte (Skillbaum)

Die Schiffsoberfläche wechselt in eine eigenständige, bildschirmfüllende **Forschungs-Sternenkarte** mit 53 echten Skills: dem `Prospektor-Kern` sowie vier farbigen Forschungsfamilien mit je 13 Upgrades. Der Hintergrund ist eine bewusst ruhige, dunkle Sternenfläche ohne dominante Landschafts- oder Fahrzeugmotive. Die Ansicht besitzt keinen Dialograhmen und kein dauerhaftes Detailpanel.

Jeder Skill erscheint als gleich großer quadratischer Icon-Slot auf einem festen 180-Pixel-Raster. Die Meilensteine `Mikro-Jetpack`, `Kristallradar`, `Quantenbohrer`, `Sternenfrachter` und `Gravitationskern` behalten einen kräftigeren Rahmen, aber keine abweichende Kartengröße. Das Mikro-Jetpack zeigt ausdrücklich den Panda. Ein Antippen öffnet nur ein kleines, am Skill verankertes Popover mit Name, Kurzbeschreibung, Kosten und Lernaktion. Voraussetzungen oder technische Abhängigkeitslisten werden dort nicht eingeblendet.

Der Forschungsgraph besteht aus vier klar getrennten Entscheidungsnetzen. Jede Familie beginnt mit einem Einstieg und teilt sich danach in drei parallele Spezialisierungen. Viele Folgeskills verwenden alternative `any`-Voraussetzungen und können deshalb aus mindestens zwei Richtungen erreicht werden; ausgewählte Meisterschaftsknoten führen zwei Wege mit `all` wieder zusammen. Der jeweilige Capstone besitzt erneut zwei alternative Endrouten. Zusätzlich existieren bewusst gesetzte Abkürzungen zwischen Forschungsfamilien. Alle Knoten liegen auf demselben 180-Pixel-Raster. Jede sichtbare Verbindung läuft als eine einzige gerade Linie zum direkten Nachbarknoten der nächsten Stufe; es gibt keine rechtwinkligen Umwege oder Zickzack-Segmente. Familienübergreifende Brücken werden erst hervorgehoben, wenn einer ihrer Endpunkte ausgewählt ist. Alte Upgrade-IDs, gespeicherte Käufe, Kosten und Gameplayeffekte bleiben kompatibel. Käufe werden weiterhin zentral im `PlayerStateManager` validiert.
