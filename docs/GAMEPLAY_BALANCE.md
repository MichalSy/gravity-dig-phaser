...
Der Forschungsgraph besteht aus vier klar getrennten, orthogonalen Entscheidungsnetzen. Jede Familie beginnt mit einem Einstieg und teilt sich danach in drei parallele Spezialisierungen. Viele Folgeskills verwenden alternative `any`-Voraussetzungen und können deshalb aus mindestens zwei Richtungen erreicht werden; ausgewählte Meisterschaftsknoten führen zwei Wege mit `all` wieder zusammen. Der jeweilige Capstone besitzt erneut zwei alternative Endrouten. Zusätzlich existieren bewusst gesetzte Abkürzungen zwischen Forschungsfamilien. Alle Knoten liegen auf demselben 180-Pixel-Raster, und jede Verbindung wird waagerecht beziehungsweise senkrecht geführt. Familienübergreifende Brücken werden erst hervorgehoben, wenn einer ihrer Endpunkte ausgewählt ist. Alte Upgrade-IDs, gespeicherte Käufe, Kosten und Gameplayeffekte bleiben kompatibel. Käufe werden weiterhin zentral im `PlayerStateManager` validiert.
